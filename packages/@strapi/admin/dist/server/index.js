...
const registerPermissionActions = async () => {
  await getService("permission").actionProvider.registerMany(
    adminActions.actions
  );
};
const registerAdminConditions = async () => {
  await getService("permission").conditionProvider.registerMany(
    adminConditions.conditions
  );
};
const registerModelHooks = () => {
  const { sendDidChangeInterfaceLanguage: sendDidChangeInterfaceLanguage2 } =
    getService("metrics");
...
const syncAPITokensPermissions = async () => {
  const validPermissions =
    strapi.contentAPI.permissions.providers.action.keys();
  const permissionsInDB = await utils$1.pipeAsync(
    strapi.query("admin::api-token-permission").findMany,
    // @ts-expect-error lodash types
    fp.map("action")
  )();
  const unknownPermissions = fp.uniq(
    fp.difference(permissionsInDB, validPermissions)
  );
...
const registerAdminPanelRoute = ({ strapi: strapi2 }) => {
  ...
  const serveAdminMiddleware = async (ctx, next) => {
    ...
    ctx.type = "html";
    ctx.body = fse__default.default.createReadStream(
...
const authenticate$2 = async (ctx) => {
  ...
  const userAbility = await getService("permission").engine.generateUserAbility(
    user2
  );
...
const constants$4 = /* @__PURE__ */ Object.freeze(
...
const isReadScope = (scope) =>
  scope.endsWith("find") || scope.endsWith("findOne");
...
const authenticate$1 = async (ctx) => {
  ...
  const hoursSinceLastUsed = dateFns.differenceInHours(
    currentDate,
    dateFns.parseISO(apiToken2.lastUsedAt)
  );
...
const destroy = async () => {
  const {
    conditionProvider: conditionProvider2,
...
const validateHasPermissionsInput =
  utils$1.validateYupSchema(hasPermissionsSchema);
...
const hasPermissions = createPolicy$1({
  ...
  handler(ctx, config) {
    ...
    const permissions2 = actions2.map((action2) =>
      inputModifiers
        .find((modifier) => modifier.check(action2))
        ?.transform(action2)
    );
    const isAuthorized = permissions2.every(
      ({ action: action2, subject: subject2 }) =>
        ability2.can(action2, subject2)
    );
    return isAuthorized;
  },
});
  ...
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::marketplace.read"] },
        },
      ],
    },
  },
  ...
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::users.create"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::users.read"] },
        },
      ],
    },
  },
  ...
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::users.update"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      policies: [
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::users.delete"] },
        },
      ],
    },
  },
...
const roles$1 = [
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::roles.read"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::roles.update"] },
        },
      ],
    },
  },
  ...
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::webhooks.create"] },
        },
      ],
    },
  },
  ...
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::webhooks.delete"] },
        },
      ],
    },
  },
...
const apiTokens = [
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::api-tokens.create"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::api-tokens.read"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::api-tokens.delete"] },
        },
      ],
    },
  },
  ...
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::api-tokens.update"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::api-tokens.regenerate"] },
        },
      ],
    },
  },
...
const { UnauthorizedError: UnauthorizedError$1, ForbiddenError } =
  utils$1.errors;
...
const authenticate = async (ctx) => {
  ...
  const transferToken2 = await tokenService.getBy({
    accessKey: tokenService.hash(token2),
  });
  if (!transferToken2) {
    return { authenticated: false };
  }
  ...
  if (!fp.isNil(transferToken2.expiresAt)) {
    ...
    if (expirationDate < currentDate) {
      return {
        authenticated: false,
        error: new UnauthorizedError$1("Token expired"),
      };
    }
  }
  const hoursSinceLastUsed = dateFns.differenceInHours(
    currentDate,
    dateFns.parseISO(transferToken2.lastUsedAt)
  );
  if (hoursSinceLastUsed >= 1) {
    await strapi.query("admin::api-token").update({
  ...
  const ability2 = await getService(
    "transfer"
  ).permission.engine.generateAbility(
    transferToken2.permissions.map((action2) => ({ action: action2 }))
  );
  return {
    authenticated: true,
    ability: ability2,
    credentials: transferToken2,
  };
};
  ...
  // Transfer Tokens
  {
    ...
    config: {
      ...
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::transfer.tokens.create"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      ...
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::transfer.tokens.read"] },
        },
      ],
    },
  },
  {
    ...
    config: {
      ...
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::transfer.tokens.delete"] },
        },
      ],
    },
  },
  ...
  },
  {
    ...
    config: {
      ...
      policies: [
        ...
        {
          name: "admin::hasPermissions",
          config: { actions: ["admin::transfer.tokens.update"] },
        },
      ],
    },
  },
...
const validatePassword = (password2, hash2) =>
  bcrypt__default.default.compare(password2, hash2);
const checkCredentials = async ({ email: email2, password: password2 }) => {
  const user2 = await strapi
    .query("admin::user")
    .findOne({ where: { email: email2 } });
...
const forgotPassword = async ({ email: email2 } = {}) => {
  const user2 = await strapi
    .query("admin::user")
    .findOne({ where: { email: email2, isActive: true } });
  if (!user2) {
    return;
  }
  ...
  return strapi
    ...
    .sendTemplatedEmail(
      ...
      {
        url,
        user: ___default.default.pick(user2, [
          "email",
          "firstname",
          "lastname",
          "username",
...
const hasSuperAdminRole$1 = (user2) => {
  return (
    user2.roles.filter((role2) => role2.code === SUPER_ADMIN_CODE$3).length > 0
  );
};
...
const assignActionId = (attrs) =>
  fp.set("actionId", computeActionId(attrs), attrs);
const assignOrOmitSubCategory = (action2) => {
  const shouldHaveSubCategory = ["settings", "plugins"].includes(
    action2.section
  );
  return shouldHaveSubCategory
    ? fp.set("subCategory", action2.subCategory || "general", action2)
    : fp.omit("subCategory", action2);
};
const appliesToProperty = fp.curry((property, action2) => {
  return fp.pipe(
    fp.prop("options.applyToProperties"),
    fp.includes(property)
  )(action2);
});
const appliesToSubject = fp.curry((subject2, action2) => {
  return (
    fp.isArray(action2.subjects) && fp.includes(subject2, action2.subjects)
  );
});
...
const checkFieldsAreCorrectlyNested = (fields) => {
  ...
  for (let indexA = 0; indexA < fields.length; indexA += 1) {
    failed = fields
      ...
      .some(
        (fieldB) =>
          fieldB.startsWith(`${fields[indexA]}.`) ||
          fields[indexA].startsWith(`${fieldB}.`)
      );
    if (failed) break;
  }
  return !failed;
...
const isAPluginName = utils$1.yup
  .string()
  .test("is-a-plugin-name", "is not a plugin name", function (value) {
    return [void 0, "admin", ...Object.keys(strapi.plugins)].includes(value)
      ? true
      : this.createError({
          path: this.path,
...
const arrayOfConditionNames = utils$1.yup
  .array()
  .of(utils$1.yup.string())
  .test("is-an-array-of-conditions", "is not a plugin name", function (value) {
    ...
    return ___default.default.isUndefined(value) ||
      ___default.default.difference(value, ids).length === 0
      ? true
      : this.createError({
          path: this.path,
...
const permissionsAreEquals = (a, b) =>
  a.action === b.action &&
  (a.subject === b.subject ||
    (___default.default.isNil(a.subject) &&
      ___default.default.isNil(b.subject)));
const checkNoDuplicatedPermissions = (permissions2) =>
  !Array.isArray(permissions2) ||
  permissions2.every((permA, i) =>
    permissions2
      .slice(i + 1)
      .every((permB) => !permissionsAreEquals(permA, permB))
  );
const checkNilFields = (action2) =>
  function (fields) {
    if (fp.isNil(action2)) {
      return true;
    }
    return (
      actionDomain.appliesToProperty("fields", action2) || fp.isNil(fields)
    );
  };
const fieldsPropertyValidation = (action2) =>
  utils$1.yup
    .array()
    .of(utils$1.yup.string())
    .nullable()
...
const permission$3 = utils$1.yup
  .object()
  .shape({
    action: utils$1.yup
      .string()
    ...
    properties: utils$1.yup
      ...
      .test(
        ...
        function (properties) {
          ...
          const hasNoProperties =
            fp.isEmpty(properties) || fp.isNil(properties);
      ...
      .test(
        ...
        async function (properties = {}) {
          ...
          try {
            await fieldsPropertyValidation(action2).validate(
              properties.fields,
              {
                strict: true,
                abortEarly: false,
  ...
  .noUnknown();
const updatePermissions = utils$1.yup
  .object()
  .shape({
    permissions: utils$1.yup
      .array()
      .required()
      .of(permission$3)
      .test(
...
const sanitizeUserRoles = (role2) =>
  ___default.default.pick(role2, ["id", "name", "description", "code"]);
const sanitizeUser = (user2) => {
  return {
    ...___default.default.omit(user2, [
      "password",
      "resetPasswordToken",
      "registrationToken",
      "roles",
...
const create$5 = async (attributes) => {
  ...
  if (___default.default.has(attributes, "password")) {
    userInfo.password = await getService("auth").hashPassword(
      attributes.password
    );
  }
  ...
  const createdUser = await strapi
    .query("admin::user")
    .create({ data: user2, populate: ["roles"] });
...
const updateById = async (id, attributes) => {
  if (___default.default.has(attributes, "roles")) {
    ...
    const superAdminRole = await getService(
      "role"
    ).getSuperAdminWithUsersCount();
    const willRemoveSuperAdminRole = !utils$1.stringIncludes(
      attributes.roles,
      superAdminRole.id
    );
  ...
  if (___default.default.has(attributes, "password")) {
    const hashedPassword = await getService("auth").hashPassword(
      attributes.password
    );
...
const resetPasswordByEmail = async (email2, password$1) => {
  const user2 = await strapi
    .query("admin::user")
    .findOne({ where: { email: email2 }, populate: ["roles"] });
  if (!user2) {
...
const getLanguagesInUse = async () => {
  const users2 = await strapi
    .query("admin::user")
    .findMany({ select: ["preferedLanguage"] });
...
const addCondition = fp.curry((condition2, permission2) => {
  ...
  const newConditions = Array.isArray(conditions2)
    ? fp.uniq(conditions2.concat(condition2))
    : [condition2];
...
const removeCondition = fp.curry((condition2, permission2) => {
  return fp.set(
    "conditions",
    fp.remove(fp.eq(condition2), permission2.conditions),
    permission2
  );
});
const getProperty = fp.curry((property, permission2) =>
  fp.get(`properties.${property}`, permission2)
);
const setProperty = (property, value, permission2) => {
  return fp.set(`properties.${property}`, value, permission2);
};
const deleteProperty = (property, permission2) =>
  fp.omit(`properties.${property}`, permission2);
const create$4 = (attributes) => {
  return fp.pipe(
    fp.pick(permissionFields),
    fp.merge(getDefaultPermission())
  )(attributes);
};
const sanitizeConditions = fp.curry((provider, permission2) => {
  if (!fp.isArray(permission2.conditions)) {
    return permission2;
  }
  return permission2.conditions
    .filter((condition2) => !provider.has(condition2))
    .reduce(
  ...
  const failIndex = permissions2.findIndex(
    (permission2) =>
      !existingActions.some(
        (action2) =>
          action2.actionId === permission2.action &&
          (action2.section !== "contentTypes" ||
            action2.subjects.includes(permission2.subject))
      )
  );
  return failIndex === -1
    ? true
...
const actionsExistSchema = utils$1.yup
  .array()
  .of(
...
const validateCheckPermissionsInput = utils$1.validateYupSchema(
  checkPermissionsSchema
);
const validatedUpdatePermissionsInput = utils$1.validateYupSchema(
  validators.updatePermissions
);
const { SUPER_ADMIN_CODE: SUPER_ADMIN_CODE$1, CONTENT_TYPE_SECTION } =
  constants$3;
...
const sanitizeRole = fp.omit(["users", "permissions"]);
...
const arePermissionsEqual = (p1, p2) => {
  if (p1.action === p2.action) {
    return deepEqual__default.default(
      jsonClean(pickComparableFields(p1)),
      jsonClean(pickComparableFields(p2))
    );
  }
  return false;
};
const create$3 = async (attributes) => {
  ...
  const autoGeneratedCode = `${___default.default.kebabCase(
    attributes.name
  )}-${utils$1.generateTimestampCode()}`;
  const rolesWithCode = {
    ...attributes,
...
const findAllWithUsersCount = async (params) => {
  const roles2 = await strapi.entityService.findMany("admin::role", params);
...
const update$3 = async (params, attributes) => {
  ...
  if (
    ___default.default.has(params, "id") &&
    ___default.default.has(sanitizedAttributes, "name")
  ) {
...
const deleteByIds$1 = async (ids = []) => {
  ...
  const deletedRoles = [];
  for (const id of ids) {
    const deletedRole = await strapi
      .query("admin::role")
      .delete({ where: { id } });
    if (deletedRole) {
...
const getUsersCount = async (roleId) => {
  return strapi
    .query("admin::user")
    .count({ where: { roles: { id: roleId } } });
};
const getSuperAdmin = () => findOne({ code: SUPER_ADMIN_CODE$1 });
const getSuperAdminWithUsersCount = () =>
  findOneWithUsersCount({ code: SUPER_ADMIN_CODE$1 });
const createRolesIfNoneExist = async () => {
  ...
  const editorPermissions = getService(
    "content-type"
  ).getPermissionsWithNestedFields(contentTypesActions, {
    restrictedSubjects: ["plugin::users-permissions.user"],
  });
  const authorPermissions = editorPermissions
    .filter(({ action: action2 }) => action2 !== ACTIONS.publish)
    .map((permission2) =>
...
const assignPermissions = async (roleId, permissions2 = []) => {
  ...
  const internalActions = getService("permission")
    ...
    .filter((action2) => action2.section === "internal")
  ...
  const permissionsWithRole = permissions2
    .map(assignRole)
    .map(permissionDomain.create);
  ...
  const permissionsToDelete = fp
    ...
    .filter((permission2) => !internalActions.includes(permission2.action));
  const permissionsToReturn = fp.differenceBy(
    "id",
    permissionsToDelete,
    existingPermissions
  );
  if (permissionsToDelete.length > 0) {
    await getService("permission").deleteByIds(
      permissionsToDelete.map(fp.prop("id"))
    );
  }
  ...
  if (
    !isSuperAdmin &&
    (permissionsToAdd.length || permissionsToDelete.length)
  ) {
...
const addPermissions = async (roleId, permissions2) => {
  const { conditionProvider: conditionProvider2, createMany: createMany2 } =
    getService("permission");
  const { sanitizeConditions: sanitizeConditions2 } = permissionDomain;
  const permissionsWithRole = permissions2
    .map(fp.set("role", roleId))
    .map(sanitizeConditions2(conditionProvider2))
    .map(permissionDomain.create);
  return createMany2(permissionsWithRole);
};
const isContentTypeAction = (action2) =>
  action2.section === CONTENT_TYPE_SECTION;
const resetSuperAdminPermissions = async () => {
  ...
  const contentTypesActions = allActions.filter((action2) =>
    isContentTypeAction(action2)
  );
  const otherActions = allActions.filter(
    (action2) => !isContentTypeAction(action2)
  );
  const permissions2 =
  ...
  const otherPermissions = otherActions.reduce((acc, action2) => {
    ...
    if (fp.isArray(subjects)) {
      acc.push(
        ...subjects.map((subject2) =>
          permissionDomain.create({ action: actionId, subject: subject2 })
        )
      );
    } else {
  ...
  const transformedPermissions =
    await hooks.willResetSuperAdminPermissions.call(permissions2);
  ...
  return new passportLocal.Strategy(
    ...
    (email2, password2, done) => {
      return getService("auth")
        .checkCredentials({ email: fp.toLower(email2), password: password2 })
        .then(async ([error, user2, message]) => {
          if (middleware) {
...
const keyIsValidEventName = ([key]) => {
  return Object.keys(strapi.admin.services.passport.authEventsMapper).includes(
    key
  );
};
...
const registerAuthEvents = () => {
  ...
  const { authEventsMapper: authEventsMapper2 } =
...
const init = () => {
  strapi.admin.services.passport
    ...
    .forEach((strategy) => passport__default.default.use(strategy));
  registerAuthEvents();
...
const token$3 = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
...
const registerProviderActionSchema = utils$1.yup
  .array()
  .required()
  .of(
    utils$1.yup
      ...
      .shape({
        uid: utils$1.yup
          ...
          .matches(
            /^[a-z]([a-z|.|-]+)[a-z]$/,
            (v) =>
              `${v.path}: The id can only contain lowercase letters, dots and hyphens.`
          )
          .required(),
        section: utils$1.yup
          .string()
          .oneOf(["contentTypes", "plugins", "settings", "internal"])
          .required(),
...
const validateRegisterProviderAction = utils$1.validateYupSchemaSync(
  registerProviderActionSchema
);
...
const createSanitizeHelpers = ({
  ...
  const createSanitizeQuery = (options = {}) => {
    ...
    const permittedFields = fields.shouldIncludeAll
      ? null
      : getQueryFields(fields.permitted);
    const sanitizeFilters = utils$1.pipeAsync(
      utils$1.traverse.traverseQueryFilters(
        removeDisallowedFields(permittedFields),
        { schema }
      ),
      utils$1.traverse.traverseQueryFilters(omitDisallowedAdminUserFields, {
        schema,
      }),
    ...
    const sanitizeSort = utils$1.pipeAsync(
      ...
      utils$1.traverse.traverseQuerySort(omitDisallowedAdminUserFields, {
        schema,
      }),
    ...
    const sanitizePopulate = utils$1.pipeAsync(
      utils$1.traverse.traverseQueryPopulate(
        removeDisallowedFields(permittedFields),
        { schema }
      ),
      utils$1.traverse.traverseQueryPopulate(omitDisallowedAdminUserFields, {
        schema,
      }),
    ...
    const sanitizeFields = utils$1.pipeAsync(
      utils$1.traverse.traverseQueryFields(
        removeDisallowedFields(permittedFields),
        { schema }
      ),
  ...
  const createSanitizeOutput = (options = {}) => {
    ...
    const permittedFields = fields.shouldIncludeAll
      ? null
      : getOutputFields(fields.permitted);
    return utils$1.pipeAsync(
      ...
      utils$1.traverseEntity(removeDisallowedFields(permittedFields), {
        schema,
      }),
  ...
  const createSanitizeInput = (options = {}) => {
    ...
    const permittedFields = fields.shouldIncludeAll
      ? null
      : getInputFields(fields.permitted);
    return utils$1.pipeAsync(
      // Remove fields hidden from the admin
      utils$1.traverseEntity(omitHiddenFields, { schema }),
      // Remove not allowed fields (RBAC)
  ...
  const wrapSanitize = (createSanitizeFunction) => {
    const wrappedSanitize = async (data, options = {}) => {
      if (fp.isArray(data)) {
        return Promise.all(
          data.map((entity) => wrappedSanitize(entity, options))
        );
      }
      const { subject: subject2, action: actionOverride } = getDefaultOptions(
        data,
        options
      );
      const permittedFields = extra.permittedFieldsOf(
        ability$1,
        actionOverride,
        subject2,
        {
      ...
      const hasAtLeastOneRegistered = fp.some(
        ...
        fp.flatMap(
          ...
          ability$1.rulesFor(
            actionOverride,
            ability.detectSubjectType(subject2)
          )
        )
      );
      const shouldIncludeAllFields =
        fp.isEmpty(permittedFields) && !hasAtLeastOneRegistered;
  ...
  const getDefaultOptions = (data, options) => {
    return fp.defaults(
      { subject: ability.subject(model, data), action: action2 },
      options
    );
  };
  ...
  const omitHiddenFields = ({ key, schema: schema2 }, { remove }) => {
    const isHidden = fp.getOr(
      false,
      ["config", "attributes", key, "hidden"],
      schema2
    );
    if (isHidden) {
      remove(key);
  ...
  const omitDisallowedAdminUserFields = (
    { key, attribute, schema: schema2 },
    { remove }
  ) => {
    if (
      schema2.uid === "admin::user" &&
      attribute &&
...
const createValidateHelpers = ({
  ...
  const createValidateQuery = (options = {}) => {
    ...
    const validateFilters = utils$1.pipeAsync(
      utils$1.traverse.traverseQueryFilters(
        throwDisallowedFields(permittedFields),
        { schema }
      ),
      utils$1.traverse.traverseQueryFilters(throwDisallowedAdminUserFields, {
        schema,
      }),
    ...
    const validateSort = utils$1.pipeAsync(
      ...
      utils$1.traverse.traverseQuerySort(throwDisallowedAdminUserFields, {
        schema,
      }),
  ...
  const createValidateInput = (options = {}) => {
    ...
    return utils$1.pipeAsync(
      ...
      // Remove not allowed fields (RBAC)
      // @ts-expect-error lodash types
      utils$1.traverseEntity(throwDisallowedFields(permittedFields), {
        schema,
      }),
  ...
  const wrapValidate = (createValidateFunction) => {
    const wrappedValidate = async (data, options = {}) => {
      if (fp.isArray(data)) {
        return Promise.all(
          data.map((entity) => wrappedValidate(entity, options))
        );
      }
      const { subject: subject2, action: actionOverride } = getDefaultOptions(
        data,
...
const mapKey = (key) => {
  if (
    ___default.default.isString(key) &&
    key.startsWith("$") &&
    key in operatorsMap
  ) {
...
const unwrapDeep = (obj) => {
  if (
    !___default.default.isPlainObject(obj) &&
    !___default.default.isArray(obj)
  ) {
    return obj;
  }
  ...
  return ___default.default.reduce(
    ...
    (acc, v, k) => {
      ...
      if (___default.default.isPlainObject(v)) {
        ...
        ___default.default.setWith(
          acc,
          key,
          v.map((v2) => unwrapDeep(v2))
        );
      } else {
...
const createPermissionEngine = (params) => {
  ...
  const engine2 = permissions__default.default.engine
    .new({ providers: providers2 })
    .on("before-format::validate.permission", ({ permission: permission2 }) => {
  ...
  return {
    ...
    async generateUserAbility(user2) {
      const permissions2 = await getService("permission").findUserPermissions(
        user2
      );
    ...
    checkMany: fp.curry((ability2, permissions2) => {
      return permissions2.map(({ action: action2, subject: subject2, field }) =>
        ability2.can(action2, subject2, field)
      );
    }),
  };
};
...
const isNotInSubjects = (subjects) => (uid) =>
  !subjects.find((subject2) => subject2.uid === uid);
...
const contentTypesBase = ({ action: action2, section }) => {
...
const subjectsHandlerFor =
  (kind) =>
  ({ action: action2, section: contentTypesSection }) => {
...
const buildDeepAttributesCollection = (model) => {
  return Object.entries(model.attributes)
    .map(([attributeName, attribute]) =>
      buildNode(model, attributeName, attribute)
    )
...
const fieldsProperty = ({ action: action2, section }) => {
  const { subjects } = action2;
  section.subjects
    .filter((subject2) => subjects?.includes(subject2.uid))
    .forEach((subject2) => {
      const { uid } = subject2;
...
const createMany = async (permissions2) => {
  ...
  for (const permission2 of permissions2) {
    const newPerm = await strapi
      .query("admin::permission")
      .create({ data: permission2 });
...
const findMany = async (params = {}) => {
  const rawPermissions = await strapi
    .query("admin::permission")
    .findMany(params);
...
const filterPermissionsToRemove = async (permissions2) => {
  ...
  for (const permission2 of permissions2) {
    const { subjects, options = {} } =
      actionProvider2.get(permission2.action) || {};
    ...
    const invalidProperties = await Promise.all(
      (applyToProperties || []).map(async (property) => {
        ...
        return (
          applies &&
          fp.isNil(permissionDomain.getProperty(property, permission2))
        );
      })
    );
    const isRegisteredAction = actionProvider2.has(permission2.action);
    const hasInvalidProperties =
      fp.isArray(applyToProperties) && invalidProperties.every(fp.eq(true));
    const isInvalidSubject =
      fp.isArray(subjects) && !subjects.includes(permission2.subject);
  ...
  for (let page = 0; page < pageCount; page += 1) {
    ...
    const permissionsNeedingToBeUpdated = fp.differenceWith(
      (a, b) => {
        return (
          a.id === b.id &&
          fp.xor(a.properties.fields, b.properties.fields).length === 0
        );
      },
    ...
    await Promise.all([
      ...
      pmap__default.default(
        permissionsNeedingToBeUpdated,
        updatePromiseProvider,
        {
          concurrency: 100,
          stopOnError: true,
        }
...
const permission$2 = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
      actionProvider,
...
const getNestedFields = (
  ...
  const nonAuthorizableFields =
    utils$1.contentTypes.getNonVisibleAttributes(model);
  return ___default.default.reduce(
    model.attributes,
    (fields, attr, key) => {
      ...
      const insideExistingFields =
        existingFields && existingFields.some(fp.startsWith(fieldPath));
...
const getNestedFieldsWithIntermediate = (
  ...
  return ___default.default.reduce(
    ...
    (fields, attr, key) => {
      ...
      if (attr.type === "component") {
        const compoFields = getNestedFieldsWithIntermediate(
...
const getPermissionsWithNestedFields = (
  ...
  return actions2.reduce((permissions2, action2) => {
    ...
    for (const subject2 of validSubjects) {
      const fields = actionDomain.appliesToProperty("fields", action2)
        ? getNestedFields(strapi.contentTypes[subject2], {
...
const cleanPermissionFields = (permissions2, { nestingLevel } = {}) => {
  ...
  return permissions2.map((permission2) => {
    ...
    const possibleFields = getNestedFieldsWithIntermediate(
      strapi.contentTypes[subject2],
      {
        components: strapi.components,
        nestingLevel,
      }
...
const contentType = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
  ...
  if (!fp.isNil(roleId)) {
    ...
    if (role2.code === AUTHOR_CODE) {
      return actionProvider2
        .values()
        .filter(({ actionId }) => actionId !== PUBLISH_ACTION);
    }
  }
  return actionProvider2.values();
};
const action = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
...
const { ValidationError: ValidationError$2, NotFoundError: NotFoundError$1 } =
  utils$1.errors;
...
const assertCustomTokenPermissionsValidity = (type, permissions2) => {
  if (type !== constants$3.API_TOKEN_TYPE.CUSTOM && !fp.isEmpty(permissions2)) {
    throw new ValidationError$2(
      "Non-custom tokens should not reference permissions"
    );
  }
  ...
  if (type === constants$3.API_TOKEN_TYPE.CUSTOM) {
    ...
    if (!fp.isEmpty(invalidPermissions)) {
      throw new ValidationError$2(
        `Unknown permissions provided: ${invalidPermissions.join(", ")}`
      );
    }
  }
};
const isValidLifespan$1 = (lifespan) => {
  ...
  if (
    !fp.isNumber(lifespan) ||
    !Object.values(constants$3.API_TOKEN_LIFESPANS).includes(lifespan)
  ) {
    return false;
  }
  return true;
...
const flattenTokenPermissions$1 = (token2) => {
  ...
  return {
    ...token2,
    permissions: fp.isArray(token2.permissions)
      ? fp.map("action", token2.permissions)
      : token2.permissions,
  };
};
...
const getExpirationFields$1 = (lifespan) => {
  const isValidNumber =
    fp.isNumber(lifespan) && Number.isFinite(lifespan) && lifespan > 0;
...
const create$1 = async (attributes) => {
  ...
  if (attributes.type === constants$3.API_TOKEN_TYPE.CUSTOM) {
    await Promise.all(
      fp.uniq(attributes.permissions).map((action2) =>
...
const update$1 = async (id, attributes) => {
  const originalToken = await strapi
    .query("admin::api-token")
    .findOne({ where: { id } });
  if (!originalToken) {
  ...
  const changingTypeToCustom =
    attributes.type === constants$3.API_TOKEN_TYPE.CUSTOM &&
    originalToken.type !== constants$3.API_TOKEN_TYPE.CUSTOM;
  ...
  if (
    updatedToken.type === constants$3.API_TOKEN_TYPE.CUSTOM &&
  ...
  return {
    ...
    permissions: permissionsFromDb
      ? permissionsFromDb.map((p) => p.action)
      : void 0,
  };
};
const apiToken$2 = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
...
const permission$1 = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
      engine,
...
const generateRandomAccessKey = () =>
  crypto__default.default.randomBytes(128).toString("hex");
const validateAccessKey = (accessKey) => {
  assert__default.default(
    typeof accessKey === "string",
    "Access key needs to be a string"
  );
  assert__default.default(
    accessKey.length >= 15,
    "Access key needs to have at least 15 characters"
  );
  return accessKey;
};
...
const create = async (attributes) => {
  ...
  const result = await strapi.db.transaction(async () => {
    ...
    await Promise.all(
      fp
        .uniq(attributes.permissions)
        .map((action2) =>
          strapi
            .query(TRANSFER_TOKEN_PERMISSION_UID)
            .create({ data: { action: action2, token: transferToken2 } })
        )
    );
    const currentPermissions = await strapi.entityService.load(
      TRANSFER_TOKEN_UID,
...
const update = async (id, attributes) => {
  ...
  return strapi.db.transaction(async () => {
    ...
    if (attributes.permissions) {
      ...
      const currentPermissions = fp.map(
        "action",
...
const regenerate = async (id) => {
  const accessKey = crypto__default.default.randomBytes(128).toString("hex");
  const transferToken2 = await strapi.db.transaction(async () =>
...
const hash = (accessKey) => {
  const { hasValidTokenSalt: hasValidTokenSalt2 } =
    getService("transfer").utils;
...
const checkSaltIsDefined = () => {
  const {
    hasValidTokenSalt: hasValidTokenSalt2,
    isDisabledFromEnv: isDisabledFromEnv2,
  } = getService("transfer").utils;
...
const assertTokenPermissionsValidity = (attributes) => {
  ...
  const invalidPermissions = fp.difference(
    attributes.permissions,
    validPermissions
  );
  if (!fp.isEmpty(invalidPermissions)) {
...
const isValidLifespan = (lifespan) => {
  ...
  if (
    !fp.isNumber(lifespan) ||
    !Object.values(constants$3.TRANSFER_TOKEN_LIFESPANS).includes(lifespan)
  ) {
    return false;
  }
  return true;
...
const token$2 = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
      checkSaltIsDefined,
...
const transfer$1 = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
...
const getProjectSettings = async () => {
  ...
  const defaultProjectSettings = PROJECT_SETTINGS_FILE_INPUTS.reduce(
    (prev, cur) => {
      prev[cur] = null;
      return prev;
    },
...
const uploadFiles = async (files = {}) => {
  return Promise.all(
    Object.values(files)
      .filter((file) => file?.stream instanceof fs__default.default.ReadStream)
...
const updateProjectSettings$1 = async (newSettings) => {
  ...
  PROJECT_SETTINGS_FILE_INPUTS.forEach((inputName) => {
    if (
      newSettings[inputName] !== void 0 &&
      !(typeof newSettings[inputName] === "object")
    ) {
...
const projectSettings = /* @__PURE__ */ Object.freeze(
  /* @__PURE__ */ Object.defineProperty(
    {
      __proto__: null,
      deleteOldFiles,
...
const updateProjectSettings = utils$1.yup
  .object({
...
const updateProjectSettingsFiles = utils$1.yup
  .object({
...
const updateProjectSettingsImagesDimensions = utils$1.yup
  .object({
    menuLogo: logoDimensions,
    authLogo: logoDimensions,
  })
  .noUnknown();
const validateUpdateProjectSettings = utils$1.validateYupSchemaSync(
  updateProjectSettings
);
const validateUpdateProjectSettingsFiles = utils$1.validateYupSchemaSync(
  updateProjectSettingsFiles
);
const validateUpdateProjectSettingsImagesDimensions =
  utils$1.validateYupSchemaSync(updateProjectSettingsImagesDimensions);
...
const admin = {
  ...
  async init() {
    ...
    const { menuLogo, authLogo } = await getService(
      "project-settings"
    ).getProjectSettings();
  ...
  async telemetryProperties(ctx) {
    ...
    const isHostedOnStrapiCloud =
      utils$1.env("STRAPI_HOSTING", null) === "strapi.cloud";
    const numberOfAllContentTypes = ___default.default.size(
      strapi.contentTypes
    );
  ...
  async information() {
    ...
    const useYarn = await fse.exists(
      path__default.default.join(process.cwd(), "yarn.lock")
    );
    return {
      data: {
        currentEnvironment,
...
const apiTokenCreationSchema = utils$1.yup
  .object()
  .shape({
    ...
    type: utils$1.yup
      .string()
      .oneOf(Object.values(constants$3.API_TOKEN_TYPE))
      .required(),
    permissions: utils$1.yup.array().of(utils$1.yup.string()).nullable(),
    lifespan: utils$1.yup
      .number()
      .min(1)
      .oneOf(Object.values(constants$3.API_TOKEN_LIFESPANS))
      .nullable(),
  })
  .noUnknown()
  .strict();
const apiTokenUpdateSchema = utils$1.yup
  .object()
  .shape({
  ...
  .noUnknown()
  .strict();
const validateApiTokenCreationInput = utils$1.validateYupSchema(
  apiTokenCreationSchema
);
const validateApiTokenUpdateInput =
  utils$1.validateYupSchema(apiTokenUpdateSchema);
...
const apiToken$1 = {
  ...
  async update(ctx) {
    ...
    if (fp.has("name", attributes)) {
      ...
      if (
        !!nameAlreadyTaken &&
        !utils$1.stringEquals(nameAlreadyTaken.id, id)
      ) {
        throw new ApplicationError$4("Name already taken");
      }
    }
...
const userCreationSchema = utils$1.yup
  .object()
  .shape({
  ...
  .noUnknown();
const profileUpdateSchema = utils$1.yup
  .object()
  .shape({
    ...
    currentPassword: utils$1.yup
      .string()
      .when("password", (password2, schema) =>
        !fp.isUndefined(password2) ? schema.required() : schema
      )
      .notNull(),
    preferedLanguage: utils$1.yup.string().nullable(),
  })
  .noUnknown();
const userUpdateSchema = utils$1.yup
  .object()
  .shape({
    email: validators.email.notNull(),
    firstname: validators.firstname.notNull(),
  ...
  .noUnknown();
const usersDeleteSchema = utils$1.yup
  .object()
  .shape({
...
const validateProfileUpdateInput =
...
const authenticatedUser = {
  ...
  async updateMe(ctx) {
    ...
    if (currentPassword && userInfo.password) {
      const isValid = await authServer.validatePassword(
        currentPassword,
  ...
  async getOwnPermissions(ctx) {
    const {
      findUserPermissions: findUserPermissions2,
      sanitizePermission: sanitizePermission2,
    } = getService("permission");
...
const registrationSchema = utils$1.yup
  .object()
  .shape({
    registrationToken: utils$1.yup.string().required(),
    userInfo: utils$1.yup
      .object()
      .shape({
        firstname: validators.firstname.required(),
        lastname: validators.lastname.nullable(),
  ...
  .noUnknown();
const registrationInfoQuerySchema = utils$1.yup
  .object()
  .shape({
    registrationToken: utils$1.yup.string().required(),
  })
  .required()
  .noUnknown();
const adminRegistrationSchema = utils$1.yup
  .object()
  .shape({
    email: validators.email.required(),
    firstname: validators.firstname.required(),
...
const validateRegistrationInfoQuery = utils$1.validateYupSchema(
  registrationInfoQuerySchema
);
const validateAdminRegistrationInput = utils$1.validateYupSchema(
  adminRegistrationSchema
);
const forgotPasswordSchema = utils$1.yup
  .object()
  .shape({
    email: validators.email.required(),
  })
...
const validateForgotPasswordInput =
  utils$1.validateYupSchema(forgotPasswordSchema);
const resetPasswordSchema = utils$1.yup
  .object()
  .shape({
...
const validateResetPasswordInput =
  utils$1.validateYupSchema(resetPasswordSchema);
const renewToken = utils$1.yup
  .object()
  .shape({ token: utils$1.yup.string().required() })
  .required()
  .noUnknown();
const validateRenewTokenInput = utils$1.validateYupSchema(renewToken);
const { ApplicationError: ApplicationError$3, ValidationError } =
  utils$1.errors;
const authentication = {
  login: compose__default.default([
    (ctx, next) => {
      return passport__default.default.authenticate(
        "local",
        { session: false },
        (err, user2, info) => {
          if (err) {
  ...
  async registrationInfo(ctx) {
    ...
    const registrationInfo = await getService("user").findRegistrationInfo(
      registrationToken
    );
    if (!registrationInfo) {
...
const permission = {
  ...
  async getAll(ctx) {
    const {
      sectionsBuilder: sectionsBuilder2,
      actionProvider: actionProvider2,
      conditionProvider: conditionProvider2,
    } = getService("permission");
...
const roleCreateSchema = utils$1.yup
  .object()
  .shape({
    name: utils$1.yup.string().min(1).required(),
    description: utils$1.yup.string().nullable(),
  ...
  .noUnknown();
const rolesDeleteSchema = utils$1.yup
  .object()
  .shape({
    ids: utils$1.yup
      .array()
      .of(utils$1.yup.strapiID())
      .min(1)
      .required()
      .test(
  ...
  .noUnknown();
const roleDeleteSchema = utils$1.yup
  .strapiID()
  .required()
  .test(
...
const roleUpdateSchema = utils$1.yup
  .object()
  .shape({
...
const role = {
  ...
  async findAll(ctx) {
    ...
    const permissionsManager = getService(
      "permission"
    ...
    const roles2 = await getService("role").findAllWithUsersCount(
      sanitizedQuery
    );
    ctx.body = {
      data: roles2,
  ...
  async getPermissions(ctx) {
    ...
    const permissions2 = await permissionService.findMany({
      where: { role: { id: role2.id } },
    });
    const sanitizedPermissions = permissions2.map(
      permissionService.sanitizePermission
    );
    ctx.body = {
      // @ts-expect-error - transform response type to sanitized permission
  ...
  async updatePermissions(ctx) {
    ...
    const permissions2 = await roleService.assignPermissions(
      role2.id,
      input.permissions
    );
    const sanitizedPermissions = permissions2.map(
  ...
  async deleteOne(ctx) {
    ...
    const sanitizedRole =
      roles2.map((role2) => roleService.sanitizeRole(role2))[0] || null;
    return ctx.deleted({
      data: sanitizedRole,
    });
  },
...
const transferTokenCreationSchema = utils$1.yup
  .object()
  .shape({
    name: utils$1.yup.string().min(1).required(),
    description: utils$1.yup.string().optional(),
    permissions: utils$1.yup
      ...
      .of(
        utils$1.yup
          .string()
          .oneOf(Object.values(constants$3.TRANSFER_TOKEN_TYPE))
      )
      .required(),
    lifespan: utils$1.yup
      .number()
      .min(1)
      .oneOf(Object.values(constants$3.TRANSFER_TOKEN_LIFESPANS))
      .nullable(),
  })
  .noUnknown()
  .strict();
const transferTokenUpdateSchema = utils$1.yup
  .object()
  .shape({
    name: utils$1.yup.string().min(1).notNull(),
    description: utils$1.yup.string().nullable(),
  ...
  .noUnknown()
  .strict();
const validateTransferTokenCreationInput$1 = utils$1.validateYupSchema(
  transferTokenCreationSchema
);
const validateTransferTokenUpdateInput$1 = utils$1.validateYupSchema(
...
const { validateTransferTokenCreationInput, validateTransferTokenUpdateInput } =
  token$1;
const token = {
  async list(ctx) {
    const transferService = getService("transfer");
...
const prefixActionsName = (prefix, dict) =>
  fp.mapKeys((key) => `${prefix}-${key}`, dict);
const transfer = {
...
const webhookValidator = utils$1.yup
  .object({
      ...
      .test(
        ...
        async (url) => {
          ...
          try {
            ...
            const isLocalUrl = await isLocalhostIp__default.default(
              parsedUrl.hostname
            );
            return !isLocalUrl;
          } catch {
...
const rateLimit =
  ...
  async (ctx, next) => {
    ...
    if (rateLimitConfig.enabled === true) {
      ...
      const requestPath = fp.isString(ctx.request.path)
        ? fp
            .toLower(path__default.default.normalize(ctx.request.path))
            .replace(/\/$/, "")
        : "invalidPath";
      const loadConfig = {
        interval: { min: 5 },
...
const dataTransfer = () => async (ctx, next) => {
  ...
  const {
    hasValidTokenSalt: hasValidTokenSalt2,
    isDataTransferEnabled: isDataTransferEnabled2,
    isDisabledFromEnv: isDisabledFromEnv2,
  } = transferUtils;
  ...
  throw new Error(
    "Unexpected error while trying to access a data transfer route"
  );
};
const index = {
  rateLimit,
