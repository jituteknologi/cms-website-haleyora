...
const syncAPITokensPermissions = async () => {
  ...
  const unknownPermissions = uniq(
    difference(permissionsInDB, validPermissions)
  );
...
const authenticate = async (ctx) => {
  ...
  const hoursSinceLastUsed = differenceInHours(
    currentDate,
    parseISO(transferToken2.lastUsedAt)
  );
  if (hoursSinceLastUsed >= 1) {
    await strapi.query("admin::api-token").update({
  ...
  const ability = await getService(
    "transfer"
...
const assignOrOmitSubCategory = (action2) => {
  ...
  return shouldHaveSubCategory
    ? set("subCategory", action2.subCategory || "general", action2)
    : omit("subCategory", action2);
};
...
const arrayOfConditionNames = yup
  ...
  .test("is-an-array-of-conditions", "is not a plugin name", function (value) {
    ...
    return _$1.isUndefined(value) || _$1.difference(value, ids).length === 0
      ? true
      : this.createError({
          path: this.path,
...
const permissionsAreEquals = (a, b) =>
  a.action === b.action &&
  (a.subject === b.subject || (_$1.isNil(a.subject) && _$1.isNil(b.subject)));
...
const sanitizeUserRoles = (role2) =>
  _$1.pick(role2, ["id", "name", "description", "code"]);
...
const updateById = async (id, attributes) => {
  if (_$1.has(attributes, "roles")) {
    ...
    const willRemoveSuperAdminRole = !stringIncludes(
      attributes.roles,
      superAdminRole.id
    );
...
const addCondition = curry((condition2, permission2) => {
  ...
  const newConditions = Array.isArray(conditions2)
    ? uniq(conditions2.concat(condition2))
    : [condition2];
...
const removeCondition = curry((condition2, permission2) => {
  return set(
    "conditions",
    remove(eq(condition2), permission2.conditions),
    permission2
  );
});
const getProperty = curry((property, permission2) =>
  get(`properties.${property}`, permission2)
);
...
const deleteProperty = (property, permission2) =>
  omit(`properties.${property}`, permission2);
...
const sanitizeConditions = curry((provider, permission2) => {
...
const validatedUpdatePermissionsInput = validateYupSchema(
...
const create$3 = async (attributes) => {
  ...
  const autoGeneratedCode = `${_$1.kebabCase(
    attributes.name
  )}-${generateTimestampCode()}`;
  const rolesWithCode = {
    ...attributes,
...
const assignPermissions = async (roleId, permissions2 = []) => {
  ...
  const permissionsToReturn = differenceBy(
    "id",
    permissionsToDelete,
    existingPermissions
  );
  if (permissionsToDelete.length > 0) {
    await getService("permission").deleteByIds(
      permissionsToDelete.map(prop("id"))
    );
  }
  ...
  return new Strategy(
    ...
    (email2, password2, done) => {
      return getService("auth")
        .checkCredentials({ email: toLower(email2), password: password2 })
...
const init = () => {
  strapi.admin.services.passport
    ...
    .forEach((strategy) => passport$1.use(strategy));
  registerAuthEvents();
...
const validateRegisterProviderAction = validateYupSchemaSync(
...
const createSanitizeHelpers = ({ action: action2, ability, model }) => {
  ...
  const createSanitizeQuery = (options = {}) => {
    ...
    const sanitizeFilters = pipeAsync(
      traverse.traverseQueryFilters(removeDisallowedFields(permittedFields), {
        schema,
      }),
    ...
    const sanitizeSort = pipeAsync(
      traverse.traverseQuerySort(removeDisallowedFields(permittedFields), {
        schema,
      }),
    ...
    const sanitizePopulate = pipeAsync(
      traverse.traverseQueryPopulate(removeDisallowedFields(permittedFields), {
        schema,
      }),
    ...
    const sanitizeFields = pipeAsync(
      traverse.traverseQueryFields(removeDisallowedFields(permittedFields), {
        schema,
      }),
  ...
  const wrapSanitize = (createSanitizeFunction) => {
    const wrappedSanitize = async (data, options = {}) => {
      ...
      const hasAtLeastOneRegistered = some(
        ...
        flatMap(
          prop("fields"),
          ability.rulesFor(actionOverride, detectSubjectType(subject2))
        )
      );
      const shouldIncludeAllFields =
        isEmpty(permittedFields) && !hasAtLeastOneRegistered;
  ...
  const getDefaultOptions = (data, options) => {
    return defaults(
      { subject: subject$1(model, data), action: action2 },
      options
    );
  };
  ...
  const pickAllowedAdminUserFields = (
    { attribute, key, value },
    { set: set2 }
  ) => {
...
const createValidateHelpers = ({ action: action2, ability, model }) => {
  ...
  const createValidateQuery = (options = {}) => {
    ...
    const validateFilters = pipeAsync(
      traverse.traverseQueryFilters(throwDisallowedFields(permittedFields), {
        schema,
      }),
    ...
    const validateSort = pipeAsync(
      traverse.traverseQuerySort(throwDisallowedFields(permittedFields), {
        schema,
      }),
    ...
    const validateFields = pipeAsync(
      traverse.traverseQueryFields(throwDisallowedFields(permittedFields), {
        schema,
      }),
...
const createPermissionEngine = (params) => {
  ...
  const engine2 = permissions$1.engine
  ...
  return {
    ...
    checkMany: curry((ability, permissions2) => {
      return permissions2.map(({ action: action2, subject: subject2, field }) =>
        ability.can(action2, subject2, field)
      );
    }),
  };
};
  ...
  for (const permission2 of permissions2) {
    ...
    const invalidProperties = await Promise.all(
      (applyToProperties || []).map(async (property) => {
        ...
        return (
          applies && isNil(permissionDomain.getProperty(property, permission2))
        );
      })
    );
    const isRegisteredAction = actionProvider2.has(permission2.action);
    const hasInvalidProperties =
      isArray(applyToProperties) && invalidProperties.every(eq(true));
    const isInvalidSubject =
      isArray(subjects) && !subjects.includes(permission2.subject);
  ...
  for (let page = 0; page < pageCount; page += 1) {
    ...
    const permissionsNeedingToBeUpdated = differenceWith(
      (a, b) => {
        return (
          a.id === b.id &&
          xor(a.properties.fields, b.properties.fields).length === 0
        );
      },
  ...
  return _$1.reduce(
    ...
    (fields, attr, key) => {
      ...
      const insideExistingFields =
        existingFields && existingFields.some(startsWith(fieldPath));
...
const flattenTokenPermissions$1 = (token2) => {
  ...
  return {
    ...token2,
    permissions: isArray(token2.permissions)
      ? map("action", token2.permissions)
      : token2.permissions,
  };
};
...
const getExpirationFields$1 = (lifespan) => {
  const isValidNumber =
    isNumber(lifespan) && Number.isFinite(lifespan) && lifespan > 0;
...
const create$1 = async (attributes) => {
  ...
  if (attributes.type === constants$3.API_TOKEN_TYPE.CUSTOM) {
    await Promise.all(
      uniq(attributes.permissions).map((action2) =>
...
const updateProjectSettingsImagesDimensions = yup
  ...
  .noUnknown();
const validateUpdateProjectSettings = validateYupSchemaSync(
  updateProjectSettings
);
const validateUpdateProjectSettingsFiles = validateYupSchemaSync(
...
const admin = {
  ...
  async telemetryProperties(ctx) {
    ...
    const isHostedOnStrapiCloud =
      env("STRAPI_HOSTING", null) === "strapi.cloud";
  ...
  .shape({
    ...
    currentPassword: yup
      ...
      .when("password", (password2, schema) =>
        !isUndefined(password2) ? schema.required() : schema
      )
      .notNull(),
    preferedLanguage: yup.string().nullable(),
  })
...
const validateRegistrationInfoQuery = validateYupSchema(
  registrationInfoQuerySchema
);
const validateAdminRegistrationInput = validateYupSchema(
...
const authentication = {
  login: compose([
    (ctx, next) => {
      return passport$1.authenticate(
        "local",
        { session: false },
        (err, user2, info) => {
          if (err) {
...
const transferTokenCreationSchema = yup
  ...
  .shape({
    ...
    permissions: yup
      .array()
      .min(1)
      .of(yup.string().oneOf(Object.values(constants$3.TRANSFER_TOKEN_TYPE)))
      .required(),
    lifespan: yup
      .number()
      .min(1)
...
const transferTokenUpdateSchema = yup
  ...
  .noUnknown()
  .strict();
const validateTransferTokenCreationInput$1 = validateYupSchema(
  transferTokenCreationSchema
);
const validateTransferTokenUpdateInput$1 = validateYupSchema(
