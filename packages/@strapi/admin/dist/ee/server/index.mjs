...
async function migrateWorkflowsContentTypes({ oldContentTypes, contentTypes }) {
  ...
  if (!hadContentTypes && hasContentTypes) {
    ...
    const contentTypes2 = pipe([pickBy(get("options.reviewWorkflows")), keys])(
      oldContentTypes
    );
...
async function addPersistTables({ strapi: strapi2 }, tableNames) {
  ...
  const notPersistedTableNames = differenceWith(
    isEqual,
    tables,
    persistedTables
  );
...
function checkVersionThreshold(startVersion, currentVersion, thresholdVersion) {
  return (
    semver.gte(currentVersion, thresholdVersion) &&
    semver.lt(startVersion, thresholdVersion)
  );
}
...
async function migrateDeletedCTInWorkflows({ oldContentTypes, contentTypes }) {
  const deletedContentTypes =
    difference(keys(oldContentTypes), keys(contentTypes)) ?? [];
...
const getRetentionDays = (strapi2) => {
  const licenseRetentionDays =
    EE.features.get("audit-logs")?.options.retentionDays;
...
const init = () => {
  strapi.admin.services.passport
    ...
    .forEach((strategy) => passport$2.use(strategy));
  registerAuthEvents();
...
const sanitizeUserRoles = (role2) =>
  _.pick(role2, ["id", "name", "description", "code"]);
...
const enableMaximumUserCount = async (numberOfUsersToEnable) => {
  ...
  const remainingDisabledUsers = drop(
...
const disableUsersAboveLicenseLimit = async (numberOfUsersToDisable) => {
  ...
  await strapi.store.set({
    ...
    value: currentlyDisabledUsers.concat(
      map(pick(["id", "isActive"]), usersToDisable)
    ),
  });
};
...
const validateProviderOptionsUpdate = validateYupSchema(
...
const permissionsAreEquals = (a, b) =>
  a.action === b.action &&
  (a.subject === b.subject || (_.isNil(a.subject) && _.isNil(b.subject)));
    ...
    properties: yup
      ...
      .test(
        ...
        function (properties) {
          ...
          return Object.keys(properties).every((property2) =>
            applyToProperties.includes(property2)
          );
        }
      )
      .test(
...
const admin = {
  ...
  async licenseLimitInformation() {
    ...
    const data = {
      ...
      permittedSeats,
      shouldNotify,
      shouldStopCreate: isNil(permittedSeats)
        ? false
...
const validateUpdateStageOnEntity = validateYupSchema(
