...
const authenticate$2 = async (ctx) => {
  ...
  const userAbility = await getService$1(
    "permission"
  ).engine.generateUserAbility(user2);
...
const authenticate$1 = async (ctx) => {
  ...
  const hoursSinceLastUsed = dateFns.differenceInHours(
    currentDate,
    dateFns.parseISO(apiToken.lastUsedAt)
  );
...
async function migrateAuditLogsTable({ oldContentTypes, contentTypes }) {
  ...
  const hasRenamedAuditLogsTable =
    oldName === "audit_logs" && newName === "strapi_audit_logs";
  if (!hasRenamedAuditLogsTable) {
    return;
  }
  const hasAuditLogsTable = await strapi.db
    .getSchemaConnection()
    .hasTable("audit_logs");
  const hasLinkTable = await strapi.db
    .getSchemaConnection()
    .hasTable("audit_logs_user_links");
  if (!hasAuditLogsTable || !hasLinkTable) {
    return;
  }
  const auditLogsColumnInfo = await strapi.db
    .connection("audit_logs")
    .columnInfo();
  const linkColumnInfo = await strapi.db
    .connection("audit_logs_user_links")
    .columnInfo();
  if (
  ...
  await strapi.db
    .getSchemaConnection()
    .renameTable("audit_logs", "strapi_audit_logs");
  await strapi.db
    .getSchemaConnection()
...
async function migrateReviewWorkflowStagesColor({
  ...
  const hadColor =
    !!oldContentTypes?.["admin::workflow-stage"]?.attributes?.color;
...
async function migrateReviewWorkflowStagesRoles({
  oldContentTypes,
  contentTypes,
}) {
  const stageUID = "admin::workflow-stage";
  const hadRolePermissions =
    !!oldContentTypes?.[stageUID]?.attributes?.permissions;
  const hasRolePermissions =
    !!contentTypes?.[stageUID]?.attributes?.permissions;
  if (!hadRolePermissions && hasRolePermissions) {
    ...
    for (const [stageId, permissions] of Object.entries(groupedPermissions)) {
      ...
      const stagePermissions2 = await stagePermissionsService.registerMany(
        permissions
      );
      await strapi.entityService.update(STAGE_MODEL_UID, numericalStageId, {
        data: {
          permissions: stagePermissions2
            .flat()
            .map((permission2) => permission2.id),
        },
      });
    }
  }
...
async function migrateWorkflowsContentTypes({ oldContentTypes, contentTypes }) {
  const hadContentTypes =
    !!oldContentTypes?.[WORKFLOW_MODEL_UID]?.attributes?.contentTypes;
  const hasContentTypes =
    !!contentTypes?.[WORKFLOW_MODEL_UID]?.attributes?.contentTypes;
  if (!hadContentTypes && hasContentTypes) {
    await strapi
      .query(WORKFLOW_MODEL_UID)
      .updateMany({ data: { contentTypes: [] } });
    const contentTypes2 = fp.pipe([
    ...
    if (contentTypes2.length) {
      await strapi
        ...
        .update({
          ...
          data: { contentTypes: contentTypes2 },
        });
    }
  }
}
...
async function addPersistTables({ strapi: strapi2 }, tableNames) {
  ...
  const notPersistedTableNames = fp.differenceWith(
    fp.isEqual,
    tables,
    persistedTables
  );
...
function checkVersionThreshold(startVersion, currentVersion, thresholdVersion) {
  return (
    semver__default.default.gte(currentVersion, thresholdVersion) &&
    semver__default.default.lt(startVersion, thresholdVersion)
  );
}
async function migrateStageAttribute({ oldContentTypes, contentTypes }) {
  ...
  const migrationNeeded = checkVersionThreshold(
    oldRWVersion,
    currentRWVersion,
    "1.1.0"
  );
  if (migrationNeeded) {
    ...
    const tables = await findTables(
      { strapi },
      new RegExp(oldAttributeTableName)
    );
    await utils$1.mapAsync(tables, async (tableName) => {
      const newTableName = tableName.replace(
        oldAttributeTableName,
        newAttributeTableName
      );
      const alreadyHasNextTable = await strapi.db.connection.schema.hasTable(
        newTableName
      );
      if (alreadyHasNextTable) {
        const dataInTable = await strapi.db
...
const getVisibleContentTypesUID = fp.pipe([
  ...
  fp.pickBy(
    (value) =>
      fp.getOr(true, "pluginOptions.content-manager.visible", value) &&
      !fp.getOr(false, "options.noStageAttribute", value)
  ),
  // Get UIDs
  fp.keys,
]);
...
async function migrateDeletedCTInWorkflows({ oldContentTypes, contentTypes }) {
  const deletedContentTypes =
    fp.difference(fp.keys(oldContentTypes), fp.keys(contentTypes)) ?? [];
  if (deletedContentTypes.length) {
    await utils$1.mapAsync(
      deletedContentTypes,
      async (deletedContentTypeUID) => {
...
const getRetentionDays = (strapi2) => {
  const licenseRetentionDays =
    EE__default.default.features.get("audit-logs")?.options.retentionDays;
...
const createAuditLogsService = (strapi2) => {
  ...
  return {
    async register() {
      ...
      state.provider = await localProvider__default.default.register({
        strapi: strapi2,
      });
      if (!EE__default.default.features.isEnabled("audit-logs")) {
        return this;
      }
      state.eventHubUnsubscribe = strapi2.eventHub.subscribe(
...
function contentTypeMiddleware(strapi2) {
  const moveReviewWorkflowOption = (ctx) => {
    const { reviewWorkflows: reviewWorkflows2, ...contentType } =
      ctx.request.body.contentType;
    if (typeof reviewWorkflows2 === "boolean") {
      ctx.request.body.contentType = fp.set(
...
const register = async ({ strapi: strapi2 }) => {
  const auditLogsIsEnabled = strapi2.config.get(
    "admin.auditLogs.enabled",
    true
  );
  if (auditLogsIsEnabled) {
    strapi2
      .hook("strapi::content-types.beforeSync")
      .register(migrateAuditLogsTable);
  ...
  if (EE__default.default.features.isEnabled("review-workflows")) {
    strapi2
      .hook("strapi::content-types.beforeSync")
      .register(migrateStageAttribute);
    strapi2
    ...
    await reviewWorkflowService.register(
      EE__default.default.features.get("review-workflows")
    );
  }
...
const registerPermissionActions = async () => {
  await getService$1("permission").actionProvider.registerMany(
    adminActions.actions
  );
};
const registerAdminConditions = async () => {
  await getService$1("permission").conditionProvider.registerMany(
      ...
      config: {
        ...
        policies: [
          "admin::isAuthenticatedAdmin",
          {
            ...
            config: { actions: ["admin::provider-login.read"] },
          },
        ],
      },
    },
    {
      ...
      config: {
        ...
        policies: [
          ...
          {
            name: "admin::hasPermissions",
            config: { actions: ["admin::provider-login.update"] },
          },
        ],
      },
    },
...
const isSsoLocked = async (user2) => {
  ...
  const roles2 =
    ...
    (await strapi
      .query("admin::user")
      .load(user2, "roles", { roles: { fields: ["id"] } })) ?? // If the query fails somehow, default to an empty array
    [];
...
const errorMessage =
  "SSO is disabled. Its functionnalities cannot be accessed.";
...
const getPassportStrategies = () => {
  ...
  const strategies = providers.map((provider) =>
    provider.createStrategy(strapi)
  );
...
const hasSuperAdminRole = (user2) => {
  return (
    user2.roles.filter((role2) => role2.code === SUPER_ADMIN_CODE$2).length > 0
  );
};
...
const updateEEDisabledUsersList = async (id, input) => {
  const disabledUsers = await getService(
    "seat-enforcement"
  ).getDisabledUserList();
  if (!disabledUsers) {
    return;
  ...
  if (user2.isActive !== input.isActive) {
    const newDisabledUsersList = disabledUsers.filter(
      (user22) => user22.id !== Number(id)
    );
...
const removeFromEEDisabledUsersList = async (ids) => {
  ...
  const newDisabledUsersList = disabledUsers.filter(
    (user2) => !idsToCheck.includes(user2.id)
  );
  await strapi.store.set({
    type: "ee",
    key: "disabled_users",
...
const enableMaximumUserCount = async (numberOfUsersToEnable) => {
  ...
  const remainingDisabledUsers = fp.drop(
    numberOfUsersToEnable,
    orderedDisabledUsers
  );
  await strapi.store.set({
    type: "ee",
    key: "disabled_users",
...
const disableUsersAboveLicenseLimit = async (numberOfUsersToDisable) => {
  ...
  const nonSuperAdminUsersToDisable = await strapi.db
    .query("admin::user")
    .findMany({
      where: {
        isActive: true,
  ...
  if (nonSuperAdminUsersToDisable.length < numberOfUsersToDisable) {
    const superAdminUsersToDisable = await strapi.db
      .query("admin::user")
      .findMany({
        where: {
          isActive: true,
  ...
  await strapi.store.set({
    ...
    value: currentlyDisabledUsers.concat(
      fp.map(fp.pick(["id", "isActive"]), usersToDisable)
    ),
  });
};
...
const seatEnforcementWorkflow = async () => {
  ...
  const currentActiveUserCount = await getService(
    "user"
  ).getCurrentActiveUserCount();
...
const workflowsContentTypesFactory = ({ strapi: strapi2 }) => {
  ...
  return {
    ...
    async migrate({ srcContentTypes = [], destContentTypes, stageId }) {
      ...
      const { created, deleted } = diffContentTypes(
        srcContentTypes,
        destContentTypes
      );
      await utils$1.mapAsync(
        created,
        async (uid) => {
          const srcWorkflows = await workflowsService._getAssignedWorkflows(
            uid,
            {}
          );
          if (srcWorkflows.length) {
            ...
            await utils$1.mapAsync(srcWorkflows, (srcWorkflow) =>
...
const workflows$1 = ({ strapi: strapi2 }) => {
  ...
  const workflowsValidationService = getService("review-workflows-validation", {
    strapi: strapi2,
  });
  ...
  return {
    ...
    async find(opts = {}) {
      ...
      return strapi2.entityService.findMany(WORKFLOW_MODEL_UID, {
        ...opts,
        filters,
        populate,
      });
    ...
    findById(id, opts) {
      const populate = processPopulate(opts.populate);
      return strapi2.entityService.findOne(WORKFLOW_MODEL_UID, id, {
        ...opts,
        populate,
      });
    },
    ...
    async create(opts) {
      ...
      return strapi2.db.transaction(async () => {
        const stages2 = await getService("stages", {
          strapi: strapi2,
      ...
      return strapi2.db.transaction(async () => {
        if (opts.data.stages) {
          ...
          opts.data.stages.forEach((stage) =>
            this.assertStageBelongsToWorkflow(stage.id, workflow2)
          );
          updatedStageIds = await stageService
            ...
            .then((stages2) => stages2.map((stage) => stage.id));
        ...
        if (opts.data.contentTypes) {
          await workflowsContentTypes.migrate({
            ...
            destContentTypes: opts.data.contentTypes,
            stageId: updatedStageIds
              ? updatedStageIds[0]
              : workflow2.stages[0].id,
          });
        }
        metrics2.sendDidEditWorkflow();
        return strapi2.entityService.update(
          WORKFLOW_MODEL_UID,
          workflow2.id,
          updateOpts
        );
    ...
    async delete(workflow2, opts) {
      ...
      return strapi2.db.transaction(async () => {
        ...
        return strapi2.entityService.delete(
          WORKFLOW_MODEL_UID,
          workflow2.id,
          opts
        );
    ...
    async _getAssignedWorkflows(uid, opts = {}) {
      return this.find({
        ...opts,
        filters: {
          contentTypes: getWorkflowContentTypeFilter({ strapi: strapi2 }, uid),
        },
      });
    },
    /**
    ...
    assertStageBelongsToWorkflow(stageId, workflow2) {
      ...
      if (!belongs) {
        throw new ApplicationError$4(
          `Stage does not belong to workflow "${workflow2.name}"`
        );
      }
    },
  };
...
const stages$1 = ({ strapi: strapi2 }) => {
  ...
  return {
    ...
    async createMany(stagesList, { fields } = {}) {
      ...
      const stages2 = await Promise.all(
        stagesList.map((stage) =>
          strapi2.entityService.create(STAGE_MODEL_UID, {
    ...
    async update(srcStage, destStage) {
      ...
      const stage = await strapi2.entityService.update(
        STAGE_MODEL_UID,
        stageId,
        {
          data: {
    ...
    async delete(stage) {
      ...
      const deletedStage = await strapi2.entityService.delete(
        STAGE_MODEL_UID,
        stage.id
      );
    ...
    async replaceStages(srcStages, destStages, contentTypesToMigrate = []) {
      const { created, updated, deleted } = getDiffBetweenStages(
        srcStages,
        destStages
      );
    ...
    async updateEntity(entityInfo, stageId) {
      ...
      const entity = await strapi2.entityService.update(
        entityInfo.modelUID,
        entityInfo.id,
        {
    ...
    async updateEntitiesStage(contentTypeUID, { fromStageId, toStageId }) {
      ...
      return strapi2.db.transaction(async ({ trx }) => {
        ...
        const selectStatement = strapi2.db
          .getConnection()
...
function getDiffBetweenStages(sourceStages, comparisonStages) {
  const result = comparisonStages.reduce(
    // ...
    (acc, stageToCompare) => {
      const srcStage = sourceStages.find(
        (stage) => stage.id === stageToCompare.id
      );
      if (!srcStage) {
  ...
  result.deleted = sourceStages.filter(
    (srcStage) =>
      !comparisonStages.some((cmpStage) => cmpStage.id === srcStage.id)
  );
  return result;
}
function assertAtLeastOneStageRemain(workflowStages, diffStages) {
  const remainingStagesCount =
    ...
    diffStages.deleted.length +
    diffStages.created.length;
...
const assignees$1 = ({ strapi: strapi2 }) => {
  ...
  return {
    ...
    async updateEntityAssignee(id, model, assigneeId) {
      ...
      const userExists = await getService("user", { strapi: strapi2 }).exists({
        id: assigneeId,
      });
      if (!userExists) {
        throw new ApplicationError$1(`Selected user does not exist`);
      }
      metrics2.sendDidEditAssignee(
        await this.findEntityAssigneeId(id, model),
        assigneeId
      );
...
const MAX_JOIN_TABLE_NAME_SUFFIX =
  1 + ENTITY_STAGE_ATTRIBUTE.length + "_links_inv_fk".length;
const MAX_CONTENT_TYPE_NAME_LEN =
  MAX_DB_TABLE_NAME_LEN - MAX_JOIN_TABLE_NAME_SUFFIX;
...
function extendReviewWorkflowContentTypes({ strapi: strapi2 }) {
  const extendContentType = (contentTypeUID) => {
    ...
    const setRelation = (path2, target) =>
      fp.set(path2, {
        writable: true,
        private: false,
        configurable: false,
    ...
    strapi2.container
      .get("content-types")
      .extend(contentTypeUID, extendContentTypeIfCompatible);
  };
  fp.pipe([
...
const registerWebhookEvents = async ({ strapi: strapi2 }) =>
  Object.entries(webhookEvents).forEach(([eventKey, event]) =>
    strapi2.webhookStore.addAllowedEvent(eventKey, event)
  );
const reviewWorkflows = ({ strapi: strapi2 }) => {
  ...
  return {
    ...
    async register({ options } = { options: {} }) {
      ...
      strapi2
        .hook("strapi::content-types.afterSync")
        .register(persistStagesJoinTables({ strapi: strapi2 }));
...
const reviewWorkflowsValidation = ({ strapi: strapi2 }) => {
  return {
    ...
    async validateWorkflowCountStages(workflowId, countAddedStages = 0) {
      ...
      if (
        countWorkflowStages + countAddedStages >
        this.limits.stagesPerWorkflow
      ) {
        throw new ValidationError$1(ERRORS.STAGES_LIMIT);
      }
    },
    ...
    async validateWorkflowCount(countAddedWorkflows = 0) {
      ...
      if (
        countWorkflows + countAddedWorkflows >
        this.limits.numberOfWorkflows
      ) {
...
const decorator = (service) => ({
  ...
  async update(uid, entityId, opts = {}) {
    ...
    if (
      updatedStage &&
      previousStage?.id &&
      previousStage.id !== updatedStage.id
    ) {
...
const reviewWorkflowsWeeklyMetrics = ({ strapi: strapi2 }) => {
  ...
  const getMetricsStoreValue = async () => {
    const value = await strapi2.store.get({
      type: "plugin",
      name: "ee",
      key: "metrics",
    });
  ...
  const setMetricsStoreValue = (value) =>
    strapi2.store.set({ type: "plugin", name: "ee", key: "metrics", value });
  return {
    ...
    async sendMetrics() {
      ...
      await setMetricsStoreValue({
        ...metricsInfoStored,
        lastWeeklyUpdate: /* @__PURE__ */ new Date().getTime(),
      });
    },
    async ensureWeeklyStoredCronSchedule() {
      const metricsInfoStored = await getMetricsStoreValue();
      const { weeklySchedule: currentSchedule, lastWeeklyUpdate } =
        metricsInfoStored;
      ...
      if (
        !currentSchedule ||
        !lastWeeklyUpdate ||
        lastWeeklyUpdate + ONE_WEEK < now.getTime()
      ) {
        weeklySchedule = getWeeklyCronScheduleAt(
          dateFns.add(now, { seconds: 10 })
        );
...
const validateProviderOptionsUpdate = utils$1.validateYupSchema(
...
const assignOrOmitSubCategory = (action) => {
  ...
  return shouldHaveSubCategory
    ? fp.set("subCategory", action.subCategory || "general", action)
    : fp.omit("subCategory", action);
};
...
const ssoUserCreationInputExtension = utils$1.yup
  .object()
  .shape({
...
const user = {
  async create(ctx) {
    if (!(await hasAdminSeatsAvaialble())) {
      throw new ForbiddenError(
        "License seat limit reached. You cannot create a new user"
      );
    }
    const { body } = ctx.request;
    const cleanData = {
  ...
  async update(ctx) {
    ...
    if (
      !(await hasAdminSeatsAvaialble()) &&
      !user2.isActive &&
      input.isActive
    ) {
      throw new ForbiddenError(
        "License seat limit reached. You cannot active this user"
      );
    }
...
const validateFindManySchema = utils$1.yup
  .object()
  .shape({
  ...
  .required();
const validateFindMany = utils$1.validateYupSchema(validateFindManySchema, {
  strict: false,
});
const auditLogs = {
  async findMany(ctx) {
...
const admin = {
  ...
  async licenseLimitInformation() {
    ...
    const eeDisabledUsers = await getService(
      "seat-enforcement"
    ).getDisabledUserList();
    if (eeDisabledUsers) {
    ...
    const data = {
      ...
      permittedSeats,
      shouldNotify,
      shouldStopCreate: fp.isNil(permittedSeats)
        ? false
        : currentActiveUserCount >= permittedSeats,
      licenseLimitStatus,
...
const validateUpdateStageOnEntitySchema = utils$1.yup
  .object()
  .shape({
...
const validateUpdateAssigneeOnEntitySchema = utils$1.yup
  .object()
  .shape({
    id: utils$1.yup.number().integer().min(1).nullable(),
  })
  .required();
const validateWorkflowCreate = utils$1.validateYupSchema(
  validateWorkflowCreateSchema
);
const validateUpdateStageOnEntity = utils$1.validateYupSchema(
...
const validateWorkflowUpdate = utils$1.validateYupSchema(
  validateWorkflowUpdateSchema
);
...
function formatWorkflowToAdmin(workflow2) {
  ...
  const transformStages = fp.map(
    fp.update("permissions", transformPermissions)
  );
...
const workflows = {
  ...
  async create(ctx) {
    ...
    const { sanitizeCreateInput, sanitizeOutput, sanitizedQuery } =
      getWorkflowsPermissionChecker({ strapi }, ctx.state.userAbility);
...
function sanitizeStage({ strapi: strapi2 }, userAbility) {
  const permissionChecker = strapi2
    .plugin("content-manager")
    .service("permission-checker")
