...
import type {
  HiddenCheckboxAction,
  VisibleCheckboxAction,
} from "./ContentTypeCollapses";
...
interface ConditionsModalProps extends Pick<ActionRowProps, "isFormDisabled"> {
  actions?: Array<
    ConditionAction | HiddenCheckboxAction | VisibleCheckboxAction
  >;
  headerBreadCrumbs?: string[];
  onClosed: ModalLayoutProps["onClose"];
  onToggle: () => void;
}
declare const ConditionsModal: ({
  actions,
  headerBreadCrumbs,
  isFormDisabled,
  onClosed,
  onToggle,
}: ConditionsModalProps) => import("react/jsx-runtime").JSX.Element;
