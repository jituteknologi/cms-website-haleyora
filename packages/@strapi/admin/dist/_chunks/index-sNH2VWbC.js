...
const build$1 = ({ command, ctx }) => {
  command
    ...
    .option(
      "--bundler [bundler]",
      "Bundler to use (webpack or vite)",
      "webpack"
    )
    ...
    .action(async (options) => {
      const { build: build2 } = await Promise.resolve().then(() =>
        require("./build-QRFarm0x.js")
      );
...
const develop$1 = ({ command, ctx }) => {
  command
    ...
    .action(async (options) => {
      const { develop: develop2 } = await Promise.resolve().then(() =>
        require("./develop-YNTGWKMC.js")
      );
...
const checkRequiredDependencies = async ({ cwd, logger, ignorePrompts }) => {
  ...
  const { install, review } = Object.entries(PEER_DEPS).reduce(
    (acc, [name, version]) => {
      if (!pkg.packageJson.dependencies) {
        throw new Error(
          `Could not find dependencies in package.json at path: ${cwd}`
        );
      }
  ...
  if (install.length > 0) {
    logger.info(
      ...
      os__default.default.EOL,
      install
        .map(({ name, wantedVersion }) => `  - ${name}@${wantedVersion}`)
  ...
  if (review.length) {
    ...
    for (const dep of review) {
      ...
      try {
        minDeclaredVersion = semver__default.default.minVersion(
          dep.declaredVersion
        );
      } catch (err) {}
      ...
      if (!installedVersion) {
        ...
        !semver__default.default.satisfies(installedVersion, dep.wantedVersion)
      ) {
        logger.warn(
          [
    ...
    if (errors.length > 0 && process.env.NODE_ENV === "development") {
      throw new Error(
        `${os__default.default.EOL}- ${errors.join(
          `${os__default.default.EOL}- `
        )}`
      );
    }
...
const getModule = async (name, cwd) => {
  const modulePackagePath = resolveFrom__default.default.silent(
    cwd,
    path__default.default.join(name, "package.json")
  );
  if (!modulePackagePath) {
    return null;
  }
  const file = await fs__default.default
    .readFile(modulePackagePath, "utf8")
...
const installDependencies = async (install, { cwd, logger }) => {
  ...
  const packages = install.map(
    ({ name, wantedVersion }) => `${name}@${wantedVersion}`
  );
  let result;
...
const getDocumentHTML = ({ logger, props = {} }) => {
  const result = server.renderToStaticMarkup(
    react.createElement(DefaultDocument, props)
  );
...
const convertModulePathToSystemPath = (modulePath) => {
  if (process.platform === "win32") {
    return modulePath
      .split(path__default.default.posix.sep)
...
const validatePackageHasStrapi = (pkg) =>
  "strapi" in pkg &&
  typeof pkg.strapi === "object" &&
  !Array.isArray(pkg.strapi) &&
  pkg.strapi !== null;
const validatePackageIsPlugin = (pkg) =>
...
const getEnabledPlugins = async ({ cwd, logger, runtimeDir, strapi }) => {
  ...
  logger.debug(
    "Dependencies from user's project",
    os__default.default.EOL,
    deps
  );
  ...
  for (const [userPluginName, userPluginConfig] of Object.entries(
    userPluginsFile
  )) {
    if (userPluginConfig.enabled && userPluginConfig.resolve) {
      ...
      plugins[userPluginName] = {
         ...
         */
        modulePath: convertSystemPathToModulePath(
          path__default.default.relative(runtimeDir, sysPath)
        ),
        path: sysPath,
      };
    }
...
const loadUserPluginsFile = async (root) => {
  for (const file of PLUGIN_CONFIGS) {
    ...
    if (configFile) {
      return typeof configFile === "function"
        ? configFile({ env: utils.env })
        : configFile;
    }
  }
  return {};
    ...
    .filter((plugin) => {
      ...
      try {
        const isLocalPluginWithLegacyAdminFile =
          plugin.path &&
          fs__default$1.default.existsSync(
            path__default.default.join(plugin.path, "strapi-admin.js")
          );
        if (!isLocalPluginWithLegacyAdminFile) {
          const isModuleWithFE = require.resolve(
            `${plugin.modulePath}/strapi-admin`
          );
  ...
  for (const file of ADMIN_APP_FILES) {
    ...
    if (await pathExists(filePath)) {
      return {
        path: filePath,
        modulePath: convertSystemPathToModulePath(
          path__default.default.relative(runtimeDir, filePath)
        ),
      };
    }
  }
...
const createBuildContext = async ({
  ...
  const strapiInstance =
    strapi ??
    strapiFactory__default.default({
      // Directories
      appDir: cwd,
  ...
  const { serverUrl, adminPath } = utils.getConfigUrls(
    strapiInstance.config,
    true
  );
  ...
  const distPath = path__default.default.join(
    strapiInstance.dirs.dist.root,
    "build"
  );
  ...
  const entry = path__default.default.relative(
    cwd,
  ...
  logger.debug(
    "Enabled plugins with FE",
    os__default.default.EOL,
    pluginsWithFront
  );
  const target =
    browserslist__default.default.loadConfig({ path: cwd }) ??
...
const build = async ({ logger, cwd, tsconfig, ignorePrompts, ...options }) => {
  ...
  contextSpinner.text = `Building build context (${prettyTime(
    contextDuration
  )})`;
  ...
  try {
    ...
    if (ctx.bundler === "webpack") {
      const { build: buildWebpack } = await Promise.resolve().then(() =>
        require("./build-bnYNP-fZ.js")
      );
      await buildWebpack(ctx);
    } else if (ctx.bundler === "vite") {
      const { build: buildVite } = await Promise.resolve().then(() =>
        require("./build-UkKhySt2.js")
      );
      await buildVite(ctx);
    }
    const buildDuration = timer.end("buildAdmin");
    buildingSpinner.text = `Building admin panel (${prettyTime(
      buildDuration
    )})`;
...
const cleanupDistDirectory = async ({ tsconfig, logger, timer }) => {
  ...
  const cleaningSpinner = logger
    .spinner(`Cleaning dist dir ${distDir}`)
    .start();
  try {
    const dirContent = await fs__default.default.readdir(distDir);
    const validFilenames = dirContent.filter(
      (filename) => filename !== "build"
    );
    for (const filename of validFilenames) {
      await fs__default.default.rm(
        path__default.default.resolve(distDir, filename),
        { recursive: true }
      );
    }
  } catch (err) {
    const generatingDuration2 = timer.end(timerName);
    cleaningSpinner.text = `Error cleaning dist dir: ${err} (${prettyTime(
      generatingDuration2
    )})`;
  ...
  cleaningSpinner.text = `Cleaning dist dir (${prettyTime(
    generatingDuration
  )})`;
...
const develop = async ({
    ...
    cluster__default.default.on("message", async (worker, message) => {
      switch (message) {
        case "reload": {
          ...
          logger.debug(
            "cluster has the reload message, sending the worker kill message"
          );
          worker.send("kill");
          break;
        }
  ...
  if (cluster__default.default.isWorker) {
    ...
    if (watchAdmin) {
      ...
      await writeStaticClientFiles(ctx);
      if (ctx.bundler === "webpack") {
        const { watch: watchWebpack } = await Promise.resolve().then(() =>
          require("./watch-IKaZxKoL.js")
        );
        bundleWatcher = await watchWebpack(ctx);
      } else if (ctx.bundler === "vite") {
        const { watch: watchVite } = await Promise.resolve().then(() =>
          require("./watch-ZVMgmXK1.js")
        );
    ...
    loadStrapiSpinner.text = `Loading Strapi (${prettyTime(
      loadStrapiDuration
    )})`;
    loadStrapiSpinner.succeed();
    if (
      tsconfig?.config ||
      strapi.config.get("typescript.autogenerate") !== false
    ) {
      ...
      generatingTsSpinner.text = `Generating types (${prettyTime(
        generatingDuration
      )})`;
    ...
    if (tsconfig?.config) {
      ...
      const compilingDuration = timer.end("compilingTS");
      compilingTsSpinner.text = `Compiling TS (${prettyTime(
        compilingDuration
      )})`;
      compilingTsSpinner.succeed();
    }
