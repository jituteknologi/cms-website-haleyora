...
const Permissions = require("./Permissions-34PVE5l6.js");
...
const CreatePage = () => {
  ...
  const { isLoading: isLoadingPermissionsLayout, data: permissionsLayout } =
    index.useGetRolePermissionLayoutQuery({
      /**
  ...
  const { data: rolePermissions, isLoading: isLoadingRole } =
    index.useGetRolePermissionsQuery(
      {
        id,
      },
      {
  ...
  const handleCreateRoleSubmit = async (data, formik2) => {
    try {
      ...
      if (
        res.data.id &&
        Array.isArray(permissionsToSend) &&
        permissionsToSend.length > 0
      ) {
        ...
        if ("error" in updateRes) {
          if (
            index.isBaseQueryError(updateRes.error) &&
            updateRes.error.name === "ValidationError"
          ) {
...
//# sourceMappingURL=CreatePage-jJLx2t7F.js.map
