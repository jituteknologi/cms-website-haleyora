...
const ExportModel = ({ model }) => {
  ...
  const downloadCsv = (filename) => {
    const file = new File([convertToCsv(content.data)], `${filename}.csv`, {
  ...
  const downloadJson = (filename) => {
    const file = new File([JSON.stringify(content)], `${filename}.json`, {
  ...
  return (
    ...
    >
      ...
      <Grid gap={4}>
        ...
        <GridItem col={2}>
          <Typography id="format-label" variant="pi">
            Export format
          </Typography>
          <RadioGroup
            ...
            value={format}
            name={`${model.uid}-format`}
          >
            <Radio value="csv">csv</Radio>
            <Radio checked={format === "json"} value="json">
              json
            </Radio>
          </RadioGroup>
        </GridItem>
